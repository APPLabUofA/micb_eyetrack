    ...
    if strcmp('on',exp.segments) == 1
        ...
        for i_part = 1:nparts
            ...
            if exp.preprocess == 2 
                if exp.corrected == 1%%%aligned and corrected, and shifted
                    ...
                    for i_event = 1:length(gopro_times)
                        if gopro_times(i_event,2) == 1
                            ...
                            EEG.event_2(i_event).code = 'Stimulus';
                ...
                elseif exp.corrected == 3
                    T = readtable(strcat('M:\Data\GoPro_Visor\Experiment_1\Video_Times\Dataframe_Whole_Vid_01_Par_', exp.participants{i_part} ,'_Frames.csv'));
                    gopro_times = table2array(T(1:height(T),[1,2]));
                    ...
                    for i_event = 1:length(gopro_times)
                        ...
                        EEG.event_2(i_event).latency = gopro_times(i_event,1);
                ...
                elseif exp.corrected == 4
                    T = readtable(strcat('M:\Data\GoPro_Visor\Experiment_1\Video_Times\Dataframe_Whole_Vid_01_Par_', exp.participants{i_part} ,'_Frames_Aligned.csv'));
                    gopro_times = table2array(T(1:height(T),[1,2]));
                ...
                elseif exp.corrected == 5
                    T = readtable(strcat('M:\Data\GoPro_Visor\Experiment_1\Video_Times\Dataframe_Whole_Vid_01_Par_', exp.participants{i_part} ,'_Frames_Transformed.csv'));
                    gopro_times = table2array(T(1:height(T),[6,3]));
                    gopro_times(:,1) = round(gopro_times(:,1).*1000);
            ...
            for i_set = 1:nsets
                ...
                nevents = length(exp.events(i_set,:));
                for i_event = 1:nevents
                    filename = [exp.name '_' exp.settings '_' exp.participants{i_part} '_' exp.setname{i_set} '_' exp.event_names{i_set,i_event} '_' exp.Corrected{exp.corrected}]
